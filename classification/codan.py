...
class CODaN_post(Dataset):
    """`CODaN <https://github.com/Attila94/CODaN>`_ Dataset with post-processing, only for test.

    Args:
        root (string): Location of the images.
        transform (callable, optional): A function/transform that takes in an PIL image
            and returns a transformed version. E.g, ``transforms.RandomCrop``
    """
    ...
    def __init__(self, root='./', transform=None):
        ...
        super(CODaN_post, self).__init__()
        ...
        # loop through class directory, load all images in memory using PIL
        for i, c in enumerate(cls_list):
            im_dir = os.path.join(root, c)
            ims = os.listdir(im_dir)
            ims = [im for im in ims if is_image_file(im)] # remove any system files
            ...
            for im in ims:
                ...
                self.paths.append(os.path.join(im_dir,im))
                self.targets.append(i)
        print('Dataset split loaded.', 'Number of images:', len(self.data))
